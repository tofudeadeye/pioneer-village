...
class BiomeWeatherGrid {
  ...
  public worldToGrid(worldX: number, worldY: number): { x: number; y: number } {
    const gridX = Math.floor(((worldX - this.MAP_MIN_X) / (this.MAP_MAX_X - this.MAP_MIN_X)) * this.width);
    const gridY = Math.floor(((worldY - this.MAP_MIN_Y) / (this.MAP_MAX_Y - this.MAP_MIN_Y)) * this.height);
   ...
   */
  private selectDiverseWeather(x: number, y: number, compatible: WeatherType[], random: () => number): WeatherType {
   ...
   */
  public getWeatherTransitionInfoAdvanced(
    worldX: number,
    worldY: number,
    heading: number,
  ): {
    ...
    const distanceFromCenter = Math.sqrt(Math.pow(normalizedDistX, 2) + Math.pow(normalizedDistY, 2));
    ...
    if (neighborGridX >= 0 && neighborGridX < this.width && neighborGridY >= 0 && neighborGridY < this.height) {
